...
- **セキュアな管理者ダッシュボード (`/admin`)**
  - Better Auth を用いた Google OAuth 認証による強力なログイン保護
...
- **Framework**: [Astro](https://astro.build/) (v6) + `@astrojs/cloudflare`
...
├── src/
│   ├── db/
│   │   └── schema.ts          # Drizzle ORMのスキーマ定義（articles, categories, user, session等）
│   ├── lib/
│   │   ├── auth.ts            # Better Auth のサーバー側インスタンス初期化
│   │   └── auth-client.ts     # Better Auth のクライアント初期化
│   ├── layouts/
...
│   │   ├── api/
│   │   │   └── auth/
│   │   │       └── [...all].ts# Better Auth のAPIエンドポイント
│   │   ├── articles/
